...
pub async fn get_account_settings(configuration: &configuration::Configuration, account_id: &str, authorization: &str, include_history: Option<bool>) -> Result<models::AccountSettingsResponse, Error<GetAccountSettingsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_account_id = account_id;
    let p_authorization = authorization;
    let p_include_history = include_history;

    let uri_str = format!("{}/v1/accounts/{account_id}/settings/identity", configuration.base_path, account_id=crate::apis::urlencode(p_account_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::AccountSettingsResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::AccountSettingsResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn update_account_settings(configuration: &configuration::Configuration, authorization: &str, if_match: &str, account_id: &str, account_settings_request: models::AccountSettingsRequest) -> Result<models::AccountSettingsResponse, Error<UpdateAccountSettingsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_authorization = authorization;
