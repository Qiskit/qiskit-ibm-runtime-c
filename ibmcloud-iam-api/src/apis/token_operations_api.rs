...
pub async fn get_token_api_key(configuration: &configuration::Configuration, grant_type: &str, apikey: &str, ibm_cloud_tenant: Option<&str>) -> Result<models::TokenResponse, Error<GetTokenApiKeyError>> {
...
pub async fn get_token_api_key_delegated_refresh_token(configuration: &configuration::Configuration, grant_type: &str, apikey: &str, response_type: &str, receiver_client_ids: &str, ibm_cloud_tenant: Option<&str>, delegated_refresh_token_expiry: Option<i32>) -> Result<models::TokenResponse, Error<GetTokenApiKeyDelegatedRefreshTokenError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_grant_type = grant_type;
    let p_apikey = apikey;
    ...
    let uri_str = format!("{}/identity/token#apikey-delegated-refresh-token", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetTokenApiKeyDelegatedRefreshTokenError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Creates a non-opaque access token for a profile.
pub async fn get_token_assume(configuration: &configuration::Configuration, grant_type: &str, access_token: Option<&str>, refresh_token: Option<&str>, cookie: Option<&str>, profile_id: Option<&str>, profile_name: Option<&str>, profile_crn: Option<&str>, account: Option<&str>) -> Result<models::TokenResponse, Error<GetTokenAssumeError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_grant_type = grant_type;
...
pub async fn get_token_cr_token(configuration: &configuration::Configuration, grant_type: &str, cr_token: &str, authorization: Option<&str>, ibm_cloud_tenant: Option<&str>, profile_id: Option<&str>, profile_name: Option<&str>, profile_crn: Option<&str>) -> Result<models::TokenResponse, Error<GetTokenCrTokenError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_grant_type = grant_type;
...
pub async fn get_token_iam_authz(configuration: &configuration::Configuration, grant_type: &str, access_token: &str, desired_iam_id: &str) -> Result<models::TokenResponse, Error<GetTokenIamAuthzError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_grant_type = grant_type;
    let p_access_token = access_token;
...
pub async fn get_token_password(configuration: &configuration::Configuration, authorization: &str, grant_type: &str, username: &str, password: &str, ibm_cloud_tenant: Option<&str>, account: Option<&str>) -> Result<models::TokenResponse, Error<GetTokenPasswordError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_authorization = authorization;
    let p_grant_type = grant_type;
