...
pub struct Circuit(*mut qiskit_ffi::QkCircuit);
...
impl Circuit {
    ...
    pub fn get_circuit_instructions(&self) -> impl ExactSizeIterator<Item = CircuitInstruction<'_>> + '_{
        let num_inst = self.num_instructions();
        (0..num_inst).map(|index| {
            unsafe {
