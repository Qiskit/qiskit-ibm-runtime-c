...
pub async fn create_service_id(
    configuration: &configuration::Configuration,
    authorization: &str,
    create_service_id_request: models::CreateServiceIdRequest,
    entity_lock: Option<&str>,
) -> Result<models::ServiceId, Error<CreateServiceIdError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ServiceId`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ServiceId`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn delete_service_id(
    configuration: &configuration::Configuration,
    id: &str,
    authorization: &str,
) -> Result<(), Error<DeleteServiceIdError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
    let p_authorization = authorization;
...
) -> Result<models::ServiceId, Error<GetServiceIdError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
    let p_authorization = authorization;
...
pub async fn list_service_ids(
    ...
    include_history: Option<bool>,
    filter: Option<&str>,
) -> Result<models::ServiceIdList, Error<ListServiceIdsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_authorization = authorization;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ServiceIdList`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ServiceIdList`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn lock_service_id(
    configuration: &configuration::Configuration,
    id: &str,
    authorization: &str,
) -> Result<(), Error<LockServiceIdError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
    let p_authorization = authorization;
...
pub async fn unlock_service_id(
    configuration: &configuration::Configuration,
    id: &str,
    authorization: &str,
) -> Result<(), Error<UnlockServiceIdError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
    let p_authorization = authorization;
...
pub async fn update_service_id(
    configuration: &configuration::Configuration,
    id: &str,
    if_match: &str,
    authorization: &str,
    update_service_id_request: models::UpdateServiceIdRequest,
) -> Result<models::ServiceId, Error<UpdateServiceIdError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
