...
/// struct for typed errors of method [`get_alive`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetAliveError {
    Status429(String),
    Status501(models::AliveResponseError),
...
pub async fn get_alive(
...
) -> Result<models::AliveResponse, Error<GetAliveError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::AliveResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::AliveResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetAliveError> = serde_json::from_str(&content).ok();
