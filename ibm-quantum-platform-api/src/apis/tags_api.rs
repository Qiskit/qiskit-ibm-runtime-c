...
pub async fn list_tags(configuration: &configuration::Configuration, r#type: &str, search: &str, ibm_api_version: Option<&str>) -> Result<models::ListTags200Response, Error<ListTagsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ListTags200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ListTags200Response`")))),
        }
    } else {
        let content = resp.text().await?;
