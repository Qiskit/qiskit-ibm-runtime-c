...
pub async fn get_account_config(configuration: &configuration::Configuration, id: &str, ibm_api_version: Option<&str>, plan_id: Option<&str>) -> Result<models::GetAccountConfig200Response, Error<GetAccountConfigError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
    let p_ibm_api_version = ibm_api_version;
    let p_plan_id = plan_id;

    let uri_str = format!("{}/v1/accounts/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetAccountConfig200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetAccountConfig200Response`")))),
        }
    } else {
        let content = resp.text().await?;
