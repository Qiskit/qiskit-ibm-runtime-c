...
/// Create a session
pub async fn create_session(configuration: &configuration::Configuration, ibm_api_version: Option<&str>, create_session_request: Option<models::CreateSessionRequest>) -> Result<models::CreateSession200Response, Error<CreateSessionError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CreateSession200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CreateSession200Response`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn delete_session_close(configuration: &configuration::Configuration, id: &str, ibm_api_version: Option<&str>) -> Result<(), Error<DeleteSessionCloseError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
    let p_ibm_api_version = ibm_api_version;

    let uri_str = format!("{}/v1/sessions/{id}/close", configuration.base_path, id=crate::apis::urlencode(p_id));
...
/// Get a session
pub async fn get_session_information(configuration: &configuration::Configuration, id: &str, ibm_api_version: Option<&str>) -> Result<models::CreateSession200Response, Error<GetSessionInformationError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
    let p_ibm_api_version = ibm_api_version;

    let uri_str = format!("{}/v1/sessions/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
...
/// Update a session
pub async fn update_session_state(configuration: &configuration::Configuration, id: &str, ibm_api_version: Option<&str>, update_session_state_request: Option<models::UpdateSessionStateRequest>) -> Result<(), Error<UpdateSessionStateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
    let p_ibm_api_version = ibm_api_version;
